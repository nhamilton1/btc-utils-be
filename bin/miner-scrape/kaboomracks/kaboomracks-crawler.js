...
const asicWattList_1 = require("../asicWattList");
const cheerio_1 = require("cheerio");
const kaboomracksScraper = async () => {
    try {
        const { data } = await axios_1.default.get("https://t.me/s/kaboomracks");
        const $miner = (0, cheerio_1.load)(data);
        const asics = [];
        $miner("body > main > div > section > div > div > div > div.tgme_widget_message_text").each((_idx, el) => {
            ...
            const moqTest = moq?.map((ele) => ele
                .split(" ")
                .map((n) => typeof n === "string" && !Number.isNaN(Number(n)) ? Number(n) : null)
                .filter((i) => i))[0];
            if (
            ...
            minerData.includes("Antminer S") &&
                ...
                let date = (0, moment_1.default)(new Date(minerData
                ...
                if (asicModel.includes("T") && !asicModel.includes("(")) {
                    let th = Number(asicModel
                        .split(" ")
                        .filter((e) => e.includes("T") && Number.isInteger(Number(e[0])) ? e[0] : null)[0]
                        .replace("T", ""));
                    ...
                    let watts = asicWattList_1.asicWattList[asicSearchName][th] !== undefined
            ...
            if (minerData.includes("Canaan A") &&
                ...
                if (asicModel.includes("T")) {
                    ...
                    let th = Number(minerData?.match(/(?=[ㄴ]\s*).*?(?=\s*Th\/s)/gs)[0]?.split(" ")[1]);
                    const asicName = minerData?.match(/(?=Canaan A\s*).*?(?=\s*for)/gs)[0];
                    let watts = asicWattList_1.asicWattList[asicName][th] !== undefined
