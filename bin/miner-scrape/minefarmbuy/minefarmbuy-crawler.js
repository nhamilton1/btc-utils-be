...
const minefarmbuyScraper = async () => {
    ...
    try {
        ...
        const urls = await page.$$eval("#blog > div > div > div > div > article > div > ul > li > a ", (title) => title.map((url) => url.href));
        ...
        for (const url of uniqUrls.values()) {
            ...
            const asicModel = await page.$eval("div > div.summary.entry-summary > h1", (el) => el.innerText);
            //checks for ddp and dap, which usually means MOQ of 100 or more from what i saw on mfb
            const incoterms = await page.$$eval("#incoterms > option", (node) => node.map((th) => th.innerText));
            //checks for efficiency dropdown when page first loads
            const effici = await page.$$eval("#efficiency > option", (node) => node.map((th) => th.innerText));
            ...
            const ifNoPriceFromAsicPrice = await page.$$eval("div > div.summary.entry-summary > p > span > bdi", (el) => el.map((e) => e.innerText));
            //checks for OoS
            //had to include this, would crash if it could not find it
            // eslint-disable-next-line no-unused-vars
            const oos = await page.$$eval("div > div.summary.entry-summary > form > p", (el) => el.map((e) => e.innerText));
            ...
            if (incoterms.length === 0 && filteredEfficiency.length === 0) {
                ...
                const hashOption = await page.$$eval("#hashrate > option", (node) => node.map((th) => th.innerText));
                ...
                for (const th of filterHashrateOptions.values()) {
                    ...
                    const asicPrice = await page.$$eval("div > div > form > div > div > div > span > span > bdi", (node) => node.map((price) => price.innerText));
                    const powerDraw = await page.$eval("#tab-additional_information > table > tbody > tr.woocommerce-product-attributes-item.woocommerce-product-attributes-item--attribute_power-draw > td", (el) => el.innerText);
                    ...
                    const asicName = asicNameFilter.match(/(?=Whatsminer\s*).*?(?=\s*abc)/gs) ||
                    ...
                    let id = `minefarmbuy ${model} ${asicPrice[0] === undefined
                ...
                for (const effic of filteredEfficiency.values()) {
                    ...
                    const hashrateOptionForEff = await page.$$eval("#hashrate > option", (node) => node.map((th) => th.innerText));
                    const efficiencyHashrateOpt = hashrateOptionForEff.filter((t) => {
                        return t.match(/th/i) && parseInt(t.charAt(0));
                    });
                    for (const th of efficiencyHashrateOpt.values()) {
                        await page.select("select#hashrate", th);
                        let asicPrice = await page.$$eval("div > div > form > div > div > div > span > span > bdi", (node) => node.map((price) => price.innerText));
                        if (asicPrice.length === 0) {
                            asicPrice = await page.$$eval("div > div.summary.entry-summary > p > span > bdi", (node) => node.map((price) => price.innerText));
                        }
                        const model = `${asicModel} ${th.split(/th/i)[0]}T ${effic.split(/j\/th/i)[0]}J/th`;
                        let id = `minefarmbuy ${model} ${Number(asicPrice[0].replace("$", "").replace(",", ""))}`;
                        minefarmbuyData.push({
                            vendor: "minefarmbuy",
                            model,
                            th: Number(th.split(/th/i)[0]),
